...
class Fare:
    ...
    def setPrice(self, price):
        ...
        if (expectedTime2Dest < 0) or (self._price > 10 * expectedTime2Dest):
            print("Fare ({0},{1}) abandoned because expectedTime2Dest was {2} and price was {3}".format(self.origin[0],
                                                                                                        self.origin[1],
                                                                                                        expectedTime2Dest,
                                                                                                        self._price))
            self._waitTime = 0
...
class NetWorld:
            ...
            for edge in edges:
                ...
                if edge.dirA < 2 or edge.dirA > 6:
                    if src.index[1] <= dst.index[1]:
                        raise ValueError(
                            "Road exit point from node {0} points away from destination node {1}".format(src.index,
                                                                                                         dst.index))
                    srcExity -= 1
                ...
                if edge.dirB < 2 or edge.dirB > 6:
                    if dst.index[1] <= src.index[1]:
                        raise ValueError(
                            "Road exit point from node {0} points away from destination node {1}".format(dst.index,
                                                                                                         src.index))
                    dstExity -= 1
                ...
                if nextNodeIdx[0] == dst.index[0] and nextNodeIdx[1] == dst.index[1]:
                    if self.addEdgeSegment(src, nextNodeIdx, edge.bidirectional) != dst:
                        raise KeyError("Indexed node ({0},{1}) should be the end junction {2}".format(nextNodeIdx[0],
                                                                                                      nextNodeIdx[1],
                                                                                                      dst.index))
                    return
                # in other cases, note that the penultimate node may not exist. In the non-bidirectional case,
                # this means we have to add it before stepping through the rest of the interpolated nodes,
                # because again, its angle to the end node might be different than the straight-line
                ...
                if not edge.bidirectional:
                    ...
                    if self.addEdgeSegment(penultimateNode, (dst.index[0], dst.index[1]), False) != dst:
                        raise KeyError(
                            "Penultimate node {1} should be adjacent to end junction {2}".format(penultimateNode.index,
                                                                                                 dst.index))
    ...
    def insertFare(self, node):
        ...
        # distribution over the node pairs, but implementing a *fast* generator of such a form is not easy.
        # Dictionaries, furthermore, have in Python 3.x the subtlety that their values() can't be indexed,
        # it returns an iterator instead, so you have to turn it into a list. It may actually be faster to grab the
        # keys(), turn those into a list, then index the element in self._net.
