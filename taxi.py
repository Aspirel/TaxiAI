...
class Taxi:
    ...
    def __init__(
        ...
        # the map is a dictionary of nodes indexed by (x,y) pair. Each entry is a dictionary of (x,y) nodes that
        # indexes a direction and distance. such a structure allows rapid lookups of any node from any other.
        ...
        if len(self._path) == 0:
            ...
            for fare in self._availableFares.items():
                ...
                if fare[1].allocated and self._passenger is None:
                    ...
                    if (
                            self._loc.index[0] == origin[0]
                            and self._loc.index[1] == origin[1]
                    ):
    ...
    def recvMsg(self, msg, **args):
        global totalTaxi100Amount, totalTaxi101Amount, totalTaxi102Amount, totalTaxi103Amount
        ...
        if msg == self.FARE_ADVICE:
            ...
            self._availableFares[
                callTime, args["origin"][0], args["origin"][1]
    ...
    def _planPath(self, origin, destination, **args):
        ...
        gCostList = {heuristic(origin, destination): {origin: [origin]}}
        while len(gCostList) > 0:
            bestPath = min(gCostList.keys())
            nextPath = gCostList[bestPath]
            if destination in nextPath:
                return nextPath[destination]
            nextNode = nextPath.popitem()
            while len(nextPath) > 0 and nextNode[0] in args["explored"]:
                nextNode = nextPath.popitem()
            if len(nextPath) == 0:
                del gCostList[bestPath]
            if nextNode[0] not in args["explored"]:
                args["explored"].add(nextNode[0])
                expansionTargets = [
                    node
                    for node in self._map[nextNode[0]].items()
                    if node[0] not in args["explored"]
                ]
                while len(expansionTargets) > 0:
                    possibleTarget = expansionTargets.pop()
                    fCost = (bestPath - heuristic(nextNode[0], destination) + possibleTarget[1][1] + heuristic(
                        possibleTarget[0],
                        destination))
                    if fCost in gCostList:
                        gCostList[fCost][possibleTarget[0]] = nextNode[1] + [possibleTarget[0]]
                    else:
                        gCostList[fCost] = {
                            possibleTarget[0]: nextNode[1] + [possibleTarget[0]]
                        }
        return None

    # ________________________________________________________________________________________________________________
    def _bidOnFare(self, time, origin, destination, price):
        taxiHasPassenger = self._passenger is not None
        TimeToNewFare = None

        # if the taxi has a passenger, the time to new fare will include the time left to drop the passenger
        # and the time from the passenger destination to the new fare location
        if taxiHasPassenger:
            newFareNode = self._world.getNode(origin[0], origin[1])
            passengerDestinationNode = self._world.getNode(self._passenger.destination[0],
                                                           self._passenger.destination[1])
            # time to new fare = time from taxi location to passenger destination + time from passenger destination
            # to new fare location
            TimeToNewFare = self._world.travelTime(self._loc, passengerDestinationNode) + self._world.travelTime(
                passengerDestinationNode, newFareNode)
        else:
            # if taxi doesn't passenger, the time to new fare is just the default from the taxi's location
            TimeToNewFare = self._world.travelTime(self._loc, self._world.getNode(origin[0], origin[1]))

        CloseEnough = self._account > TimeToNewFare
        ...
        # No point continuing if finite times are less than 0
        FiniteTime = TimeToNewFare > 0 or TimeToDestination > 0
        if not FiniteTime:
            return False

        ExpectedPay = price - TimeToDestination
        PriceBetterThanCost = price > TimeToDestination

        # is the trip even worth it? if the money in the taxis account is bigger than the time it takes the
        # taxi to reach the new fare and the fare destination + at least 5, its worth it. Also, is the price fair?
        # Does the expected payment cover the time to reach the new fare and has profit on top of it?
        # Constant 10 is a random constant to make sure the taxi has a worthwhile profit.
        WholeTripWorth = self._account > (TimeToDestination + TimeToNewFare) + 10
        FairPriceToNewFare = ExpectedPay > TimeToNewFare + 10
        # if the time to reach the fare is too long, its probable that it will cancel. This already include
        # traffic or passengers.
        WillProbablyCancel = TimeToNewFare > (self._maxFareWait - self._world.simTime + time)
        Worthwhile = FairPriceToNewFare and PriceBetterThanCost
        WillArriveOnTime = self._maxFareWait - self._world.simTime + time > TimeToNewFare
        NoAllocatedFares = len([fare for fare in self._availableFares.values() if fare.allocated]) == 0

        return not WillProbablyCancel and CloseEnough and WholeTripWorth and WillArriveOnTime and Worthwhile \
               and NoAllocatedFares
